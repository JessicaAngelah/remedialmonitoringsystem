...
# Blank starter workbooks for the Import Excel modal — one per import
# "kind" — so users importing Master data / a Weekly update / a Single
# case have a correctly-shaped file to fill in rather than guessing at
# columns. Files live alongside uploads/ at the project root.
IMPORT_TEMPLATES_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "import_templates")
IMPORT_TEMPLATE_FILES = {
    "master": ("MasterData_Template.xlsx", "Master Data Template.xlsx"),
    "weekly": ("Weekly_Report_Template.xlsx", "Weekly Report Template.xlsx"),
    "single": ("SingleCase_Template.xlsx", "Single Case Template.xlsx"),
}
...
def project_detail(pid):
    ...
    # Set only when we arrived here from a folder page, so the back link
    # (and the links down into each task) can return the user to that same
    # folder instead of always bouncing out to "All projects".
...
@projects_bp.get("/import/template/<kind>")
def download_import_template(kind):
    """Serve the blank starter workbook for the given import kind (master /
    weekly / single), so the Import Excel modal can offer a correctly
    shaped file instead of leaving users to guess at columns."""
    entry = IMPORT_TEMPLATE_FILES.get(kind)
    if not entry:
        abort(404)
    stored_name, download_name = entry
    return send_file(
        os.path.join(IMPORT_TEMPLATES_DIR, stored_name),
