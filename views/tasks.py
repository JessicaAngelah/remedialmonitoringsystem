...
def export_tasks_excel():
    """Excel download of the full Tasks list — always exports every task
    regardless of whatever search/filter/sort is currently applied on the
    page, and always reads straight from the live TASKS list, so any edit
    made on a task's detail page (title, status, dates, notes, etc.) is
    already reflected the moment this is downloaded — there's no separate
...
def task_detail(tid):
    ...
    # Carried over from project_detail's own from_folder so the back link
    # here can send the user all the way back to the folder they started
    # in, not just to the project (which would itself then default to
    # "All projects" and drop the folder).
    ...
    return render_template(
        ...
        is_overdue_task=is_overdue(tk), from_folder=from_folder,
    )
